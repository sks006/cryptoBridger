...
// In your borrow.rs (or wherever borrow logic lives)
...
pub fn borrow(ctx: Context<Borrow>, amount: u64) -> Result<()> {
    ...
    // MVP simplification: Assume 1 SOL ≈ X EUR using oracle
    let collateral_value_eur: u128 = 
        ((user_position.deposited_amount as u128) * (sol_price.price as u128))
        / 10u128.pow(sol_price.exponent.unsigned_abs() as u32)
        / (eur_price.price as u128)
        * 1_000_000;   // Scale to 6 decimals (EURC-like)

    let debt_value_eur: u128 = user_position.borrowed_amount as u128; // must be in 6 decimals
    ...
    msg!("Borrow successful: {} EURC. New HF: {}", amount, health_factor);
